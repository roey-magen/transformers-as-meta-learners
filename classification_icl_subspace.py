...
class GaussianMixtureDataset(Dataset):
    ...
    def __init__(self, d: int, N: int, B: int, R: float, is_validation: bool=False, label_flip_p: float =0.0, k: int = 10):
        self.d = d
        self.k = k
        self.N = N 
        self.B = B
        ...
        if is_validation:
            # use differents seed for validation data
...
class Trainer:
    ...
    def __init__(self, config: ExperimentConfig):
        ...
        self.train_dataset = GaussianMixtureDataset(
            config.d, config.N, config.B, config.R_train, is_validation=False, label_flip_p=0.0, k= config.k # no noise during pre-training
        )
        self.val_dataset = GaussianMixtureDataset(
            config.d, config.N, config.B, config.R_val, is_validation=True, label_flip_p=config.label_flip_p, k= config.k
        )
    ...
    def train(self):
        ...
        print(f"Subspace dimension (k): {self.config.k}")
        print(f"Training tasks: {self.config.B}")
        print(f"Validation tasks: {self.config.B_val}")
        print(f"Batch size(Number of tasks): {self.config.B}")
        print("-" * 50)
...
def run_single_experiment(params, base_results_dir: str, use_cuda: bool = False):
    ...
    """
    d, B, R_train, R_val, steps, k = params
    ...
    experiment_name = f"d{d}_k{k}_B{B}_R{R_train}_{timestamp}"
...
def run_parallel_cpu_experiments(num_processes: int = None):
    ...
    for B in B_list:
        param_combinations.append((d, B, R_train, d**0.3, steps, k))
...
if __name__ == "__main__":
    ...
    USE_CPU = True
    run_single_experiment_cpu = False
    different_k_experiments = True
    ...
    if torch.cuda.is_available() and not USE_CPU:
    ...
    elif run_single_experiment_cpu:
        print('Running single experiment with CPU')
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_results_dir = f"results_{timestamp}"
        ...
        k = 200
        R_train = 5 * k ** 0.5
        params = (d, B, R_train, d ** 0.35, 500, k)
        run_single_experiment(params=params, base_results_dir=base_results_dir, use_cuda=True)

    elif different_k_experiments and not run_single_experiment_cpu:
        print('Running experiment for different k with CPU')
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_results_dir = f"results_{timestamp}"
        k_list = [30]
        ...
        for k in k_list:
            for R in R_list:
                # R_train = d ** R_d_to_power

                params = (d, B, R, R, 300, k)
                run_single_experiment(params=params, base_results_dir=base_results_dir, use_cuda=True)

    elif different_k_experiments and run_single_experiment_cpu:
        print('Only one flag can set to true among run_single_experiment_cpu and different_k_experiment, please change that')
